...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance __UNUSED,
                   LPSTR lpCmdLine __UNUSED, int nCmdShow __UNUSED)
{
    HWND hwnd;
    MSG uMsg;
    if (!SetUpWindowClass("1", 0, 0, 128, hInstance)) /* Navy blue */
        return 1;
    hwnd = CreateWindow("1", 0, WS_BORDER, 0, 0, 100, 100, NULL, NULL,
                        hInstance, NULL);
...
int SetUpWindowClass(char *Title, int bgRed, int bgGreen, int bgBlue,
                     HINSTANCE hInst)
{
...
void __cdecl draw_proc(void *phdc)
{
    HWND hWnd = phdc;
    char *Text[] = {
    ...
    font = CreateFont(winrect.bottom / 48, winrect.right / 84, 0, 0, 0, 0, 0, 0,
    ...
    if (firstdraw) /* Draw a increasing process if it's the first draw */
    {
        ...
        for (int count = 5; count <= 100; count += 5)
        {
            ...
            while (1)
                if ((int)((clock() - t) / CLOCKS_PER_SEC) >= 1)
                    break;
            snprintf(dumpstatus, 37, "Dumping physical memory to disk: %d",
                     count);
            TextOut(hDC, 5, horipos - horiinc, dumpstatus, strlen(dumpstatus));
        }
    }
    TextOut(hDC, 5, horipos - horiinc, "Dumping physical memory to disk: 100",
            37);
...
LRESULT CALLBACK WindowProcedure(HWND hWnd, unsigned int uiMsg, WPARAM wParam,
                                 LPARAM lParam)
{
    switch (uiMsg)
    {
        ...
        case WM_PAINT:
            if (curthrd == 0)
                curthrd = _beginthread(draw_proc, 0, hWnd);
            break;
        default:
